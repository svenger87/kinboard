...
### Added
- **End-to-end auto-update overlay** — `docker-compose.diun.yml.example` runs Diun (image notifier) + a webhook executor that fires `kinboard-self-update.sh` whenever a new `ghcr.io/svenger87/kinboard` digest lands on GHCR. The script runs the full upgrade sequence: `git pull` → `setup.sh --non-interactive` → `docker compose pull` → `up -d` → conditional `kong restart` if `kong.yml`'s mtime moved. Replaces the deprecated Watchtower overlay (containrrr/watchtower was archived upstream in 2024 and only handled image pulls — it never git-pulled compose/kong/migrations or re-ran setup.sh, so config-bearing releases silently broke Watchtower-driven installs). Manual trigger via `docker exec kinboard-webhook /scripts/kinboard-self-update.sh`. New `DIUN_WEBHOOK_SECRET` (auto-generated by `setup.sh`) is the HMAC token between Diun and the webhook container.
