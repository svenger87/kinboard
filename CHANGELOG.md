...
### Fixed
- Shopping PWA on iOS now opens in standalone mode again. The PWA's manifest sets `start_url` and `scope` to `/einkaufen` (a German legacy slug retained for back-compat with installed PWAs). `next.config.mjs`'s German-slug normalizer was 308-redirecting `/einkaufen → /shopping` — but the resulting URL is OUTSIDE the PWA's `/einkaufen` scope, so iOS detects a scope violation and bounces the launched page to Safari instead of keeping it in PWA standalone mode. Converted that redirect to a rewrite so the browser URL stays at `/einkaufen` while internally serving the `/shopping` page. The other German-slug redirects (`/energie`, `/hausautomation`, `/kameras`, `/mahlzeiten`, `/notizen`, `/rezepte`) stay as redirects since none of them are PWA-scoped.
