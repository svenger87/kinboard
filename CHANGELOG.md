...
### Changed
- **Shopping PWA URL stays at `/einkaufen` (legacy German slug) but the historic 793-line route file is gone.** The original plan was to drop `/einkaufen` entirely and route to `/shopping`, but iOS caches the installed PWA's manifest scope at install time and doesn't refetch — so changing the manifest broke "Open in App" scope-match for every existing installed Shopping PWA on iOS until the user manually reinstalled. New design preserves the URL alias without the duplicate code: `/einkaufen` is a Next rewrite to `/shopping` content (URL stays `/einkaufen` for scope-match, content from the canonical 1291-line `/shopping/page.tsx`). The historic `webapp/src/app/einkaufen/` route directory and the `einkaufen.*` i18n namespace were both deleted (the duplicate had silently diverged from `/shopping` over time). `manifest-shopping.json` keeps `start_url` + `scope` at `/einkaufen` for installed-PWA continuity. Push-notification URLs (`/api/cron/process-notifications`, `/api/notifications/{debug-trigger,send-batch}`) also point at `/einkaufen` so notification taps open the installed PWA via the iOS scope-match banner. NO_NAV_PATHS includes both `/einkaufen` and `/shopping` since they're URL aliases of the same kiosk-style page. **No reinstall required for existing iOS users.**
