...
- A reusable "Salbei / Leinen" component library built on the redesigned primitives: PersonAvatar, PersonChip, ChecklistItem, EventPill, TodayStripPill, WidgetCard, IntegrationStatusRow, FAB, plus a shared person-color tint helper. These are the building blocks every redesigned screen composes from.
...
- Mobile navigation is now a fixed 4-target bottom bar (Home, Calendar, Shopping, More); "More" opens a bottom sheet listing every remaining route — Home Assistant- and plugin-gated entries and your per-device order are preserved, with the same unread badges.
- A kiosk status line: in kiosk mode the bottom navigation is replaced by a quiet top bar (compact weather chip on the left, notifications + settings on the right), with widget tiles and swipe as the primary navigation.
- App-wide route transitions: a calm 320 ms fade with a slight rise on navigation, reduced to a plain fade on kiosk and disabled entirely under `prefers-reduced-motion`.
...
### Changed
- Weather detail (the dashboard weather modal) restyled to the Salbei/Leinen tokens with an info-tint current-conditions hero; the radar layer switcher uses the inset-segment tabs. Screensaver clock now uses the lighter Bricolage weight, and its section labels (News/Events/Birthdays) are now localized (were previously hardcoded German).
...
- Settings hub and integration pages redesigned to the flat "Salbei/Leinen" look: live connection-status dots on the hub, a unified connection-status banner on Home Assistant / Google / Bring / Photos, and consistent page padding across all subpages.
- Redesigned the visual foundation ("Salbei / Leinen"): warm sage-linen neutrals, a single rotating monthly accent driving every `--primary` surface, new Hanken Grotesk / Bricolage Grotesque / Space Mono typography, and restyled Button/Card/Tabs/Switch/Checkbox/Badge primitives. Light and dark mode fully retuned.
...
- Restyled EmptyState (dashed linen card) and Skeleton (muted to card shimmer) and the desktop navigation to the new tokens (flat `bg-card` bar, primary-tinted active pill) — dropping the old glass/blur chrome on app surfaces.
...
- Redesigned the Energy dashboard for the Salbei/Leinen theme: flat linen cards (no glass), a re-skinned flow diagram with icon-badge nodes and marching dashed flux paths, theme-following energy-token chart colors with a dashed consumption line, a desktop stat column (yield/autarky/export), and a mobile compact Solar→Home→Grid flow with a battery bar. Honors `prefers-reduced-motion` (static dashes).
...
### Added
- **Pocket Money plugin (Piggy)** — fifth registered SurfacePlugin (after Vehicles, Energy, Cameras, Stonks). Per-kid virtual pocket-money accounts with parent-configurable APR (daily accrual + daily commit — see "Changed" below), scheduled allowance cron, multi-goal saving queue with image lookup (reuses item-catalog plumbing) + URL paste + upload, and a kid-facing avatar that visibly evolves through 8 stages as `lifetime_saved_cents` crosses thresholds (€2 / €5 / €15 / €40 / €100 / €300 / €1000), with five built-in species (dragon, cat, astronaut, plant, wizard) — each with 8 hand-picked Noto Emoji SVGs. Adding a sixth species is a 3-step config change: drop 8 SVGs into `webapp/public/pocket-money/avatars/`, add an entry to `webapp/src/plugins/pocket-money/catalog/avatars.json`, add the matching `species.<id>.tier1..tier8` i18n keys — no DB migration, no code changes (validation is catalog-driven at the application layer). Allowance cadence is configurable per-account (weekly / biweekly / every 4 weeks) with a locale-aware day-of-week picker. Three lightweight celebration animations: avatar evolution, goal reached, and interest pay — gated once-per-event by per-account state so reloads don't re-trigger. Kid actions are safe-by-default (view, add goals, propose withdrawals); destructive primitives (deposit, withdraw, edit rate, approve withdrawal) live in `/settings/pocket-money` for the parent. Reuses the existing `people.is_child` flag (no schema change to the people table) to filter who's eligible for an account. Three new Ofelia crons: `pocket-money-accrue-interest` (daily 00:30 UTC), `pocket-money-commit-interest` (hourly check, fires daily per-account with a 23h dedup window), `pocket-money-process-allowance` (hourly check, fires per-account on the configured day with a (interval-1)-day idempotency window). New idempotent migration `migration_pocket_money.sql` creates four tables + `goal-images` Supabase Storage bucket. See [Pocket Money](https://github.com/svenger87/kinboard/wiki/Pocket-Money) for the full feature reference.
- **End-to-end auto-update overlay** — `docker-compose.diun.yml.example` runs Diun (image notifier) + a webhook executor that fires `kinboard-self-update.sh` whenever a new `ghcr.io/svenger87/kinboard` digest lands on GHCR. The script runs the full upgrade sequence: `git pull --ff-only` → `setup.sh --non-interactive` → `docker compose pull --ignore-buildable` → `up -d` (excluding webhook+diun to prevent self-kill) → conditional `kong restart` if `kong.yml`'s mtime moved → conditional `diun restart` if `diun.yml`'s mtime moved. Replaces the deprecated Watchtower overlay (containrrr/watchtower was archived upstream in 2024 and only handled image pulls — it never git-pulled compose/kong/migrations or re-ran setup.sh, so config-bearing releases silently broke Watchtower-driven installs). Manual trigger via `docker exec kinboard-webhook /scripts/kinboard-self-update.sh`. New `.env` keys (auto-filled by `setup.sh`): `DIUN_WEBHOOK_SECRET` (HMAC), `KINBOARD_PROJECT_DIR` (absolute host path; webhook bind-mounts it at the same path inside so docker-compose's relative paths resolve consistently), `COMPOSE_PROJECT_NAME=kinboard` (stable network names regardless of cwd), `COMPOSE_FILES` (overlay flag set, no literal quotes). The webhook image is locally built from `Dockerfile.webhook` (Alpine + git + docker-cli + openssl + the adnanh/webhook binary) — upstream `almir/webhook` ships only the webhook binary and would be unable to `git pull` or `docker compose`.
- New `/settings/navigation` page — drag-and-drop reorder for the bottom navigation. Saved per-device in localStorage so the kitchen kiosk, parent's phone, and kids' tablets each keep their own layout. Items not in the saved order (e.g. a freshly enabled plugin) append at the end, so newly-shipped surfaces don't go missing. Reset-to-default button reverts to the hardcoded `NAV_ITEMS` sequence. Built on `framer-motion`'s `Reorder` (already a dep, used by `/settings/homeassistant/rooms`).
- `/settings/pocket-money` — the species picker now shows the full evolution strip (every stage SVG) for each species and, after a species is selected, opens an inline preview panel with all 8 stage names labelled, so parents and kids can see the full saving journey before committing to an avatar choice.
- `/settings/pocket-money` — per-account "projected balance" forecast at 1 / 3 / 6 / 12 months, simulating the cron path day-by-day (daily APR accrual + daily commit + allowance on the configured interval) so parents can dial in a realistic APR and see what the balance will look like at the current rate. Flags the cap with an inline hint when the projection bumps into `max_balance_eligible_cents`.
...
### Changed
- Pocket Money goal-add dialog: the Search tab now augments the curated `item_catalog` with a web image search (DuckDuckGo) when the catalog has fewer than 5 hits for the query — so kids can pick a picture for that brand-new LEGO set even when no one's added it to the catalog yet. Curated rows still come first; web rows are tagged with a small "web" badge and are persisted as `image_source = "url"` (matching the existing DB enum). Implementation is server-side, no API key, no per-family config; failure is non-fatal — when the upstream is unreachable the dialog quietly falls back to catalog-only.
- Pocket Money interest now commits **daily** instead of weekly. The pending interest cron still accrues daily into `pending_interest_cents`, but the commit cron no longer waits for a configured `interest_committed_day_of_week` — interest moves into the visible balance on the next hourly run within ~24h. Parents felt the weekly cadence as "interest is broken" since accrued cents sat invisible for up to 6 days. The legacy `interest_committed_day_of_week` column is preserved on existing rows for backwards-compat but is no longer consulted, and the corresponding picker on `/settings/pocket-money` is hidden. Forecast simulation in `lib/pocket-money/interest.ts:projectBalance` updated to match.
- Dashboard widgets now stretch to fill their grid cell so widgets in the same row are visually equal-height. Adds `[&>*]:h-full` to the grid section and `h-full` to each widget's root Card / Link wrapper — no more dead whitespace under the shorter widgets in a row.
- Desktop bottom-nav theme toggle: switched from `variant="outline"` to a ghost-styled rounded-full pill matching the rest of the nav items, with explicit `text-muted-foreground hover:text-foreground` so the sun/moon icon is contrast-correct in dark mode. The outline variant's border + icon both blended into the glass background in dark mode — visually invisible until hover.
- `setup.sh`'s `fill_secret` now appends a missing `KEY=` line if `.env` doesn't have one yet — previously it only replaced existing empty keys, so existing self-hosters whose `.env` predated a newly-introduced key (e.g. `DIUN_WEBHOOK_SECRET`) silently never got it.

### Deprecated
- `docker-compose.watchtower.yml.example` — keeps working for existing self-hosters but should be migrated to the Diun overlay. See [Self-hosting → Auto-updates → Watchtower migration](https://github.com/svenger87/kinboard/wiki/Self-hosting#auto-updates) for the swap-out steps.

### Fixed
- **Auto-update overlay (Diun + webhook) was silently broken since first shipping.** Diun's YAML config used Go-template syntax (`{{ env "DIUN_WEBHOOK_SECRET" }}`) for the `X-Diun-Token` header, but Diun does NOT evaluate Go templates in `notif.webhook.headers` values — it shipped the literal template string. The webhook's trigger-rule (which correctly templated `{{ getenv "..." }}` because it's started with `-template`) compared the literal token string to the resolved env var and rejected every notification with "trigger rules were not satisfied" + HTTP 200, so failures were silent. Net effect: Diun saw new images and fired notifications, the webhook acked them, but the self-update script never ran. Affected every self-hoster running the Diun overlay. Fix: `diun.yml` now uses a `REPLACE_WITH_DIUN_WEBHOOK_SECRET` placeholder that `setup.sh` substitutes from `.env` (same pattern kong.yml uses for ANON_KEY / SERVICE_ROLE_KEY). Setup.sh also self-heals stacks still on the old broken template syntax — re-running it once after this update is enough. `kinboard-self-update.sh` now restarts `kinboard-diun` when `diun.yml` mtime moves (matching its existing `kong.yml` restart pattern) so the substituted config is picked up live.
...
### Added
- **Stonks plugin** — track stocks, ETFs, crypto, indices, and forex pairs in a watchlist with a rotating dashboard widget and a per-ticker detail page with proper candle charts. Yahoo Finance is the v1 (and only) data driver via the unofficial `yahoo-finance2` library — no API key required, covers every asset class through one source. Charts via TradingView's open-source `lightweight-charts` (1d / 1w / 1m / 3m / 1y / max timeframes; up/down candles colored green/red). Server-side TTL cache (30s spot quotes, 5min charts, 1h symbol searches) keeps Yahoo from rate-limiting on dashboard auto-refresh. New `/stonks` page (per-ticker tabs), `/settings/stonks` (symbol autocomplete + watchlist manager), `tickers` table migration with `(family_id, symbol)` unique index. Fourth registered SurfacePlugin after Vehicles, Energy, Cameras — driver contract leaves room for `polygon`, `tiingo`, etc. later when paid sources matter.
...
- Manual "Sync now" button on `/settings/ics` for user-triggered single-family ICS refresh alongside the 30-min cron. Backed by `/api/calendar/sync-ics`, sharing the same `syncIcsCalendar` helper as the cron path so upsert/delete behaviour can't drift between the two entry points.
...
- Unified `/settings/calendar` landing page — a single "Calendar" entry on the main settings page now links into both the Google Calendar OAuth flow (`/settings/google`) and the ICS feed manager (`/settings/ics`) so users don't have to know which provider they're configuring before they get there. Existing direct links to either detail page keep working unchanged.

### Fixed
- `/settings/plugins` toggle list now shows the Stonks plugin's label + description text instead of the raw `settings.plugins.label.stonks` key. Same omission previously affected Energy and Cameras and was fixed in the v1.0.18 cycle; Stonks needed the matching keys added in both `messages/en.json` and `messages/de.json`.
- `/settings/calendar` no longer shows Google as "not connected" when an OAuth access token is in fact present. The connected check now uses `Boolean(googleStatus?.access_token)` since the `GoogleCalendarSettings` shape has no boolean `connected` field — presence of the token is the canonical signal.
- Lockfile-sync drift introduced by Windows `npm install` runs during the Stonks plugin work — `package-lock.json` regenerated on Linux to restore `npm ci` validity (was missing nested `@swc/helpers@0.5.21` from `next-intl`). CI lint, E2E smoke, and the Docker image build all errored at the install step before this fix.
