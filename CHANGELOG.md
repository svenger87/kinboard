...
### Changed
- **Shopping PWA URL is now `/shopping` (was `/einkaufen`).** The German legacy slug had been retained for installed-PWA back-compat across the i18n refactor, but a divergent 793-line `/einkaufen/page.tsx` and the never-quite-firing 308 redirect masked the fact that the new 1291-line `/shopping/page.tsx` (with recipes, item-catalog tags, image search, etc.) was the canonical implementation all along. Dropped the entire `/einkaufen/` route directory and the matching `einkaufen.*` i18n namespace. `manifest-shopping.json`'s `start_url` + `scope` + shortcut URL are now `/shopping`. NO_NAV_PATHS swapped from `/einkaufen` → `/shopping` so the bottom nav stays hidden in kiosk shopping mode. `next.config.mjs` still has a 308 redirect at `/einkaufen/:path* → /shopping/:path*` so direct URL hits / old bookmarks keep working. **Existing iOS-installed Shopping PWAs need to be deleted + reinstalled** from `/shopping` in Safari — their cached manifest still says `start_url: /einkaufen`, which now 308s to `/shopping` (out of the cached scope) and bounces to Safari. The reinstall is a one-time cost; after it the PWA opens cleanly in standalone with no scope-violation hops.
