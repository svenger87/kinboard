...
### Fixed
- **Auto-update overlay (Diun + webhook) was silently broken.** Diun's YAML config used Go-template syntax (`{{ env "DIUN_WEBHOOK_SECRET" }}`) for the `X-Diun-Token` header, but Diun does NOT evaluate Go templates in `notif.webhook.headers` values — it shipped the literal template string. The webhook's trigger-rule (which correctly templated `{{ getenv "..." }}` because it's started with `-template`) compared the literal token string to the resolved env var and rejected every notification with "trigger rules were not satisfied" + HTTP 200, so failures were silent. Net effect: Diun saw new images and fired notifications, the webhook acked them, but the self-update script never ran. Affects every self-hoster running the Diun overlay since it shipped. Fix: `diun.yml` now uses a `REPLACE_WITH_DIUN_WEBHOOK_SECRET` placeholder that `setup.sh` substitutes from `.env` (same pattern kong.yml uses for ANON_KEY / SERVICE_ROLE_KEY). Setup.sh also self-heals stacks still on the old broken template syntax — re-running it once after this update is enough. `kinboard-self-update.sh` now restarts `kinboard-diun` when `diun.yml` mtime moves (matching its existing `kong.yml` restart pattern) so the substituted config is picked up live.
