...
- **School holidays are entered under Settings → School schedule.** Give the period a name and a first and last day; both days count, so a single day off is the same date twice. They sit with the timetable because they answer the same question — when is there school — and the reminders they silence are the ones the timetable drives.
...
- **Deleting an appointment in Google now removes it here too.** An event cancelled in Google usually just stops appearing in what Google sends, and Kinboard only removed something when Google explicitly said it had been cancelled — which it does for a short while and then stops. So an appointment deleted weeks ago could still be sitting on the board. The sync now compares what Google sent against what it is holding and drops whatever is no longer there. It only does that for a calendar it managed to read all the way through, so a network hiccup or an unusually large calendar can never be mistaken for you having emptied it, and appointments you created in Kinboard itself are never touched. Events further than a year either side of today are outside the window the sync reads and are left alone.
- **The home screen keeps its margin on a phone.** Every card on the dashboard sat neatly away from the left edge and ran straight off the right one, because the widget area measured itself against the whole screen while sitting inside a frame that already holds a margin. On a wide display the two happen to agree, so only phones and small tablets showed it. The cards now sit evenly on both sides at every size.
- **On a phone, settings pages no longer start underneath the back button.** Every settings page reserves a strip at the top for the round back button that floats over the corner. On a screen without a notch — which is most phones, and every desktop browser — that reserved strip was being thrown away, so each page began at the very top and its icon and heading sat under the button. It only showed on narrow screens, which is why the wall display looked right.
- **Going back from a calendar sub-page returns to the calendar.** CalDAV, ICS and Google are reached through Settings → Kalender, but the back arrow on each of them jumped straight to the top of Settings — so getting back to where you were meant walking in again. It now returns one step, to the page you came from. CalDAV also had a second back arrow of its own, pointing somewhere different from the first; there is one now.
- **One back button on the settings pages, not two.** The hints page drew its own back control on top of the one every settings page already gets from the frame around it. The two sat in the same corner and overlapped, so it read as one until the page was given its proper spacing — at which point there were visibly two. The integrations page has been brought onto the same frame while we were there, so it keeps clear of that button too.
- **Adding a school holiday works.** Filling in the dialog and pressing Add did nothing at all: the database had never been told that the browser is allowed to write to the new table, so the save was refused before it began — and the screen said nothing, because that one form was the only place on the page that did not report a failed save. Both halves are fixed, so it saves, and if a save ever does fail it now says so instead of appearing to ignore the button.
- **The hints settings page looks like the rest of Settings.** It was missing the frame every other settings page uses, so the back button sat on top of the heading and hid the line explaining the page, the rows stretched the full width of a wall display instead of a readable column, and the last hint was tucked behind the navigation bar.
