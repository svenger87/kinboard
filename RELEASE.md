...
How a new Familyboard version reaches `ghcr.io` and the GitHub Releases page.

This document is the maintainer's checklist. End-users and contributors don't need to read it.

## Versioning

Familyboard follows [Semantic Versioning 2.0.0](https://semver.org/):
...
- [ ] CI is green on `familyboard/main`
...
```diff
 [Unreleased]: https://github.com/svenger87/familyboard/compare/vX.Y.Z...HEAD
+[X.Y.Z]: https://github.com/svenger87/familyboard/releases/tag/vX.Y.Z
```
...
### 4. Cherry-pick onto `familyboard` remote

`familyboard/main` is squashed-history; commits land via cherry-pick. From local main:

```bash
SHA=$(git rev-parse HEAD)
git fetch familyboard main
git checkout -b release-vX.Y.Z familyboard/main
git cherry-pick "$SHA"
```
...
```bash
git push familyboard release-vX.Y.Z:main
git push familyboard vX.Y.Z
```

The tag push triggers `.github/workflows/docker.yml`, which builds amd64 + arm64 images on the Unraid runner and publishes to:

- `ghcr.io/svenger87/familyboard:X.Y.Z`
- `ghcr.io/svenger87/familyboard:X.Y`
- `ghcr.io/svenger87/familyboard:X`
- `ghcr.io/svenger87/familyboard:latest`
...
gh release create vX.Y.Z \
  --repo svenger87/familyboard \
  --title "Familyboard vX.Y.Z" \
...
```bash
docker pull ghcr.io/svenger87/familyboard:X.Y.Z
docker manifest inspect ghcr.io/svenger87/familyboard:X.Y.Z | grep architecture
```
...
4. Cherry-pick onto `familyboard/main`, tag `vX.Y.Z+1`, push tag
...
3. Optionally, delete the `:latest` and `:X` Docker tags (but keep `:X.Y.Z` so existing pinned deployments don't break): use `gh api -X DELETE /user/packages/container/familyboard/versions/<id>`.
