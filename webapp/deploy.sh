#!/usr/bin/env bash
# Familyboard — remote deployment to a self-hosted server over SSH.
#
...
for migration in "$SCRIPT_DIR"/docker/migration*.sql; do
    ...
    $SSH_CMD "docker exec -i ${PROJECT_NAME:-familyboard}-db psql -U postgres -d postgres < /tmp/$name" || true
    $SSH_CMD "rm -f /tmp/$name"
done

# Reload PostgREST schema cache after migrations
$SSH_CMD "docker restart ${PROJECT_NAME:-familyboard}-rest 2>/dev/null" || true
...
TARBALL="/tmp/familyboard-deploy.tar.gz"
cd "$SCRIPT_DIR"
tar czf "$TARBALL" \
...
# Upload to remote
$SCP_CMD "$TARBALL" "$USER@$HOST:/tmp/familyboard-deploy.tar.gz"
...
# Extract fresh tree
$SSH_CMD "cd $REMOTE_PATH/webapp && tar xzf /tmp/familyboard-deploy.tar.gz"
$SSH_CMD "rm -f /tmp/familyboard-deploy.tar.gz"
rm -f "$TARBALL"
...
echo "========================================"
$SSH_CMD "docker ps --filter name=${PROJECT_NAME:-familyboard} --format 'table {{.Names}}\t{{.Status}}'"
