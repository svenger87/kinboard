...
BEGIN
  IF NOT EXISTS (
    ...
    CREATE TABLE public.pocket_money_accounts (
      ...
      -- avatar_species is validated at the application layer against
      -- the catalog at webapp/src/plugins/pocket-money/catalog/avatars.json.
      -- No DB CHECK so adding a new species in the catalog doesn't
      -- require a migration.
...
-- ('dragon', 'cat', 'astronaut'); validation moved to the application
-- layer so adding a new species in the catalog JSON doesn't require a
-- migration. Constraint name is auto-generated by Postgres for inline
-- CHECKs, so we look it up by introspecting pg_constraint.
DO $$
DECLARE
  cname text;
BEGIN
  ...
  WHERE t.relname = 'pocket_money_accounts'
    AND c.contype = 'c'
    AND pg_get_constraintdef(c.oid) ILIKE '%avatar_species%';

  IF cname IS NOT NULL THEN
    EXECUTE format(
      'ALTER TABLE public.pocket_money_accounts DROP CONSTRAINT %I', cname
    );
  END IF;
END $$;
