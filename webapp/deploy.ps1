# Kinboard — Windows / PowerShell deployment helper.
...
#   -ProjectName  container prefix (default kinboard)
#   -PublicUrl    URL to print on success
#
# Examples:
#   .\deploy.ps1 -RemoteHost nas.example.com -RemotePath /mnt/user/appdata/kinboard
...
param(
    ...
    [string]$ProjectName = $(if ($env:PROJECT_NAME) { $env:PROJECT_NAME } else { "kinboard" }),
...
Write-Host "Kinboard Deployment" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
