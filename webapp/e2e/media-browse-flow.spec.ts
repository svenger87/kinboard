...
/*
  The skip belongs on the describe, not inside the test. `ci.yml`'s stack-free
  Specs job installs no browsers on purpose — it uses Playwright as a plain
  test runner — so a test that takes the `page` fixture fails at
  `browserType.launch` before a skip in its body ever runs. Skipping at this
  level means the fixture is never created. Same shape as
  `automation-layout.spec.ts` and `messages-broadcast.spec.ts`.
*/
test.describe("browsing a player's library", () => {
  test.skip(!FAMILY_CODE, "needs FAMILY_CODE and a running stack");

  test("browsing drills into a folder and plays what is tapped", async ({
    page,
  }, testInfo) => {
    await establishSession(
      page,
      FAMILY_CODE,
      `claude-br-${testInfo.project.name}`,
    );
    ...
    const suffix =
      `${testInfo.project.name.replace(/[^a-z0-9]/gi, "")}${Date.now()}`.toLowerCase();
    ...
    const created = await page.evaluate(
      async ({ fid, nick, eid }) => {
        ...
        return res.ok
          ? { id: (await res.json()).player.id as string }
          : { error: `${res.status}` };
      },
    ...
    try {
      ...
      await expect(page.getByText("Probe Folder")).toBeVisible({
        timeout: 10_000,
      });
      await page.getByText("Probe Folder").click();
      await expect(page.getByText("Probe Track")).toBeVisible({
        timeout: 10_000,
      });
      ...
      await page.evaluate(
        async ({ fid, id }) => {
          await fetch(`/api/media-players/${id}?family_id=${fid}`, {
            method: "DELETE",
          }).catch(() => {});
        },
