...
git remote add origin git@github.com:svenger87/familyboard.git
...
- **Public-repo GitHub URLs** anywhere in `docs/wiki/*.md` (e.g. `github.com/svenger87/familyboard/blob/main/...`) — these point at the project's own source
- **Maintainer contact emails** in CODE_OF_CONDUCT.md + SECURITY.md
- **Sponsor handle** in FUNDING.yml + README + Home.md + _Footer.md
- **Illustrative placeholder LAN IPs** (`192.168.1.100`) in `messages/{en,de}.json` (camera URL examples) and `docs/wiki/Integration-Cameras.md`
- **Maintainer GHCR namespace** (`ghcr.io/svenger87/familyboard`) in the Docker pipeline workflow + docker-compose.image.yml
- **NAS path** (`/mnt/user/appdata/familyboard`) in `migrate-prod.sh` as a documented default — overridable via `DATA_DIR=` env var
...
- Personal hostnames (`familyboard.svenger87.de`, `10.10.10.x`, `192.168.1.x`, `/mnt/user/appdata/familyboard`, the `S8L4GQ` family code) are scrubbed from every tracked file in the launch commit (`2286841c`)
...
`familyboard.svenger87.de`, `10.10.10.x`, `192.168.1.x`, `/mnt/user/appdata/familyboard` no longer appear in any tracked code path that ships. Files touched:
...
- `kiosk/kiosk-{cage,gnome,gnome-setup,weston,x11,x11-fallback}.sh` — `${FAMILYBOARD_URL:-http://localhost:3001/}` everywhere
...
| `webapp/docker/.env.example` | `/mnt/user/appdata/familyboard` | Mentioned only as an example NAS path in a comment |
...
arm64) images to `ghcr.io/svenger87/familyboard` on every push to
...
- [/] **Fresh-machine test.** In progress on Hetzner CX23 (Ubuntu 24.04, 4 GB RAM). Real bugs caught + fixed in this session:
  ...
  16. "Family Calendar" branding still in 15 user-facing surfaces (sw.js push title, en.json/de.json title, deploy headers, etc.) → bulk rename to "Familyboard"
