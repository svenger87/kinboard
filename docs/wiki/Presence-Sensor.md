...
Optional hardware accessory: an **HLK-LD2410** (or LD2410B / LD2410C) **24 GHz radar presence sensor**, wired to the kiosk via USB-UART. The sensor reports presence + distance to a body in front of it; Familyboard uses this to:
...
Find the device ID:
1. Open Familyboard → **Settings → Devices**
...
[Unit]
Description=LD2410 presence sensor for Familyboard
...
presence-sensor.py
    ...
    │       └─► Familyboard webapp updates `devices.last_presence` in DB
...
- [`presence-sensor.py`](https://github.com/svenger87/familyboard/blob/main/presence-sensor.py) — the script itself
