...
For families that already use [Bring!](https://www.getbring.com/), the Bring integration is **optional** — connect it and Familyboard will keep its list and your Bring list in two-way sync. But the built-in shopping list works fully on its own and is the recommended path for new users (one less account, one less moving part).
...
The built-in list runs entirely on your Familyboard instance — no external service:
...
Familyboard ships with a **dedicated PWA just for the shopping list**, separate from the main Familyboard PWA. Installing it gives you:
...
This is the **recommended phone install** for any family member whose only need on their phone is the shopping list. Power user pattern: parents who do the shopping install just the shopping PWA, kids install the full Familyboard PWA.
...
If you've **already installed the main Familyboard PWA** and want the shopping one too: open `/einkaufen` directly in your browser (not via the existing PWA icon), then install — both icons coexist on the home screen.

### Main Familyboard PWA vs shopping-only PWA
...
If you've connected [[Integration-Bring|Bring!]], items sync two-way every 2 minutes:

- Add in Familyboard → appears in Bring! on family members' phones
- Add in Bring! on a phone → appears in Familyboard
...
Familyboard remembers items per family in `public.item_catalog`. When you add "Müsli," Familyboard offers the suggestion next time without recomputing the keyword match. Edits to category / unit / preferred brand stick across re-additions.

## Notifications

Shopping list pushes (per [[Notifications]]):
...
- **Phone PWA in the supermarket**: install the Familyboard PWA on your phone, walk the aisles ticking items as you grab them. Works offline, syncs as soon as you're back in range.
...
- [[Integration-Bring]] — full Bring! sync setup
