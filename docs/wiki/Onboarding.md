...
```bash
docker exec -i familyboard-db psql -U postgres -d postgres <<'EOF'
...
- **Don't enable AssignedAccess kiosk mode for the kiosk user account** if you want notifications + Edge to coexist — Familyboard's Edge `--kiosk` flag does the lockdown without the AssignedAccess complications. See [[Kiosk-Windows-11-Mele-4C]].
