...
- Doesn't record. go2rtc supports RTSP recording but Familyboard doesn't expose it.
...
- **Digest** — most modern IP cameras (Hikvision, Amcrest, Dahua). Familyboard's `/api/cameras` proxy handles digest negotiation.
...
- See [`webapp/docker/go2rtc.yaml`](https://github.com/svenger87/familyboard/blob/main/webapp/docker/go2rtc.yaml) for the streaming bridge config
- See [`webapp/src/components/camera-viewer.tsx`](https://github.com/svenger87/familyboard/blob/main/webapp/src/components/camera-viewer.tsx) for the player implementation
