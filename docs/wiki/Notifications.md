...
Familyboard supports **web push notifications** — every device that opts in gets push messages for shopping-list changes, todo deadlines, and (planned) calendar reminders.

## TL;DR — getting notifications working on a phone

1. **Install Familyboard as a PWA** (Add-to-Home-Screen). On iOS this is **mandatory** — Apple won't deliver web push to plain Safari, only to installed PWAs. On Android it's recommended for reliability.
2. **Open Familyboard via the home-screen icon** (not the browser bookmark).
...
> **Two PWAs available.** Familyboard ships *two* installable PWAs that share the same backend: the **main Familyboard PWA** (full app) and a **shopping-only PWA** scoped to the shopping list. Push subscriptions are per-origin, so both PWAs share the same notification permission and can receive any notification type. Pick whichever install fits your phone usage. See [Shopping-only PWA](#shopping-only-pwa-separate-install) below.
...
The **PWA install** is what makes Familyboard feel like a native app on phones — and it's required for push to work on iOS. Per platform:

### iOS (Safari)

1. Open Familyboard in Safari (not Chrome — iOS Chrome is just a Safari skin and won't install)
2. Tap the **Share** button (square with up-arrow)
3. Scroll down, tap **Add to Home Screen**
4. Confirm — an icon now lives on your home screen
5. Open Familyboard from that icon (not from Safari) — it launches in standalone mode without the URL bar
...
1. Open Familyboard in the browser
...
Familyboard also ships a **dedicated shopping-only PWA** with its own manifest (`/manifest-shopping.json`), green shopping-cart icon, and home-screen entry — completely separate from the main Familyboard PWA. The two coexist on the same device.

**Why it exists:** family members who do the shopping want a fast-launching phone app that opens straight to the list without nav clutter. Power-user pattern: parents who do groceries install only the shopping PWA; the kitchen kiosk + a kid's phone install the full Familyboard PWA.

**What it gives you:**

- A separate home-screen icon (green shopping-cart, distinct from the main Familyboard icon)
...
| | Main Familyboard PWA | Shopping-only PWA |
|---|---|---|
| Install from | Anywhere in Familyboard | `/einkaufen` (banner deep-links you there) |
| Manifest | `/manifest.json` | `/manifest-shopping.json` |
| Icon | Familyboard logo | Green shopping cart |
...
- Familyboard signs each push payload with the VAPID private key; the push service verifies and delivers
- The browser's service worker shows the notification, with sound, vibration, badge, etc.

VAPID keys are generated once per Familyboard instance via `setup.sh`:
...
There's also a stand-alone Tkinter GUI in `tools/notification-tester.py` for end-to-end debugging. It connects to your Familyboard URL, lists subscribed devices, lets you fire arbitrary push payloads. Useful when developing new notification types.
...
| **Test notification doesn't arrive** | VAPID subject doesn't match the URL Familyboard runs on, OR endpoints are unreachable from the host. Check webapp logs. |
| **Notifications work in browser but not in iOS PWA** | iOS requires the **standalone display mode** to be in `manifest.json`. Familyboard's manifest is correct; verify the PWA was actually installed via Add-to-Home-Screen and not just bookmarked. |
...
- [[Quick-start]] — `setup.sh` is what generates VAPID keys for you
- See [`webapp/src/lib/push-sender.ts`](https://github.com/svenger87/familyboard/blob/main/webapp/src/lib/push-sender.ts) for the server-side push code
