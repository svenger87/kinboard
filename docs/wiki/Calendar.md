...
1. **Google Calendar (two-way sync)** — any calendar you've enabled in [[Integration-Google-Calendar|Settings → Google Calendar]]. Events created or edited in Familyboard are pushed back to Google; edits made on the Google side flow down at the next sync interval. Subject to Google's per-calendar permissions: a read-only subscribed calendar (holidays, sports schedules, etc.) stays read-only in Familyboard too.
2. **Local events** — added directly in the Familyboard UI without selecting a Google calendar. Lives in `public.events` with no Google link and never round-trips to Google.
...
1. **Whole calendar → one person** — set in [[Integration-Google-Calendar|Settings → Google Calendar]] per calendar
...
- **Calendar** — pick any of your Google calendars (event will round-trip to Google) or "Local only" (stays inside Familyboard)
- Notes
...
Hidden under [[Integration-Google-Calendar|Settings → Google Calendar]]. Useful when:
...
- **Attendees / invites.** Google events show their attendees in the day panel as read-only text; you can't RSVP or invite people from Familyboard.
...
- **Editing read-only subscribed Google calendars** — if Google itself marks the calendar as read-only (someone else's calendar shared with you, public holiday calendars, etc.), Familyboard can't write to it either.

## Related

- [[Integration-Google-Calendar]] — connect Google + per-event mapping rules
