...
The top of the page has a row of room tabs (configured via [Settings → Home Assistant → Rooms](Home-Assistant#4-configure-rooms)):
...
Beyond rooms, [dashboards](Home-Assistant#3-configure-dashboards) let you curate a flat grid of cards across rooms. Use cases:
...
`/energy` is the solar / battery / grid power dashboard. Pulls live data from HA sensors mapped in [Settings → HA → Energy](Home-Assistant#5-optional-configure-the-energy-dashboard).
...
Each chart's series are configurable in [Settings → HA → Energy → Chart configuration](Home-Assistant#5-optional-configure-the-energy-dashboard) — pick which entities show up, label them, color them.
