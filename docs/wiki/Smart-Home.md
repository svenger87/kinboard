...
Two pages built on top of [Home-Assistant](Home-Assistant.md):

- **`/home-automation`** — touch-friendly room view of HA entities
- **`/energy`** — solar / battery / grid energy dashboard

This page covers what those surfaces *do*. For connecting HA, see [Home-Assistant](Home-Assistant.md).
...
The top of the page has a row of room tabs (configured via [Settings → Home Assistant → Rooms](Home-Assistant.md#room-configuration)):
...
Beyond rooms, [dashboards](Home-Assistant.md#dashboards) let you curate a flat grid of cards across rooms. Use cases:
...
`/energy` is the solar / battery / grid power dashboard. Pulls live data from HA sensors mapped in [Settings → HA → Energy](Home-Assistant.md#optional-configure-the-energy-dashboard).
...
Each chart's series are configurable in [Settings → HA → Energy → Chart configuration](Home-Assistant.md#optional-configure-the-energy-dashboard) — pick which entities show up, label them, color them.
...
- [Home-Assistant](Home-Assistant.md) — connecting HA, configuring rooms / dashboards / energy
- [Cameras](Cameras.md) — for cameras outside the HA ecosystem (direct go2rtc)
