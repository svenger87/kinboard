...
Each recipe can have multiple tags. Familyboard's catalog suggests common ones (`schnell`, `vegetarisch`, `kinderfreundlich`, etc.) but tags are free-form. Tag a recipe `meal-prep` and you can quick-filter to it later.
...
Empty cells show a subtle "Try Pizza" or similar randomized hint based on the meal type to spark ideas — see [[Themes-and-Locales|mealHints translation namespace]] for the list.
