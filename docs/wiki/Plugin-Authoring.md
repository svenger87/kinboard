...
Familyboard already has six "integrations" (Google, HA, Immich, Bring, OpenWeatherMap, Cameras). Those are core because almost everyone wants them and they generalize.
...
- Plugins live **inside** the main `webapp/src/plugins/` tree, not as separate npm packages. Familyboard is a single-deploy app; per-plugin deployment isn't a goal.
...
Don't write plugins yet. Open a [discussion](https://github.com/svenger87/familyboard/discussions) describing the integration you want — it might already be on the roadmap, or might be a stronger candidate for core inclusion than for a plugin.
