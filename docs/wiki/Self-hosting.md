...
| **You set up a domain + Traefik for HTTPS** | `https://familyboard.your-domain.com` (no port — Traefik handles 443) |
...
3. Restart Kong to pick up the new CORS allowlist: `docker restart familyboard-kong` (a `kong reload` is **not** enough — DB-less Kong only re-parses `kong.yml` on container start)
...
|---|---|---|
| `PROJECT_NAME` | `familyboard` | Container name prefix (e.g. `familyboard-db`) |
...
| `DOMAIN` | `familyboard.example.com` | Public domain — only consumed by the Traefik overlay |
...
```
DOMAIN=familyboard.example.com
...
```bash
docker exec -t familyboard-db pg_dump -U postgres -F c postgres > /backups/familyboard-$(date +%F).pgdump
```
...
```bash
ssh nas
cd /mnt/user/appdata/familyboard/webapp/docker
...
- **The `storage` container's healthcheck is flaky upstream**: it sometimes shows `unhealthy` even while serving requests fine. This is a known Supabase issue, not a Familyboard regression.
