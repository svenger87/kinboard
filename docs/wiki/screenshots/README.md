...
Familyboard demo stack seeded with anonymized prod data, with mock external
...
────────                              ───────────
  ┌─────────────────┐                 ┌──────────────────────┐
  │ familyboard-db  │                 │ docker stack         │
  │ (your prod      │ ──── pg_dump ──►│  ↓                   │
  │  Familyboard    │                 │  postgres (demo)     │
...
├── demo.env(.example)          # env: PROJECT_NAME=familyboard-demo, ports, secrets
...
Implements just enough of the HA REST + WebSocket API for Familyboard to
...
- WebSocket `/api/websocket` — minimal `auth` + `subscribe_events` +
  `get_states` support so Familyboard's live-update path works
...
docker compose --project-name familyboard-demo restart webapp
npm run capture
./scripts/embed.sh
```
