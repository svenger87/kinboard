...
Familyboard pings every device every 60 seconds while it's open (the heartbeat in `auth-guard.tsx`). Devices that haven't pinged in 5 minutes show as offline.

Devices that haven't pinged in 30+ days are good candidates to delete from the list. Familyboard doesn't auto-prune.
