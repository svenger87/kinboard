...
For the **full build** — BOM, vendor links, frame construction, cabling, photos — see [[Reference-Build]]. This page covers the **software** that runs on top.
...
Boot → AutoLogon as Calendar
  ...
  │       ├─ 2s delay
  │       └─ Launches Edge in --kiosk mode pointing at your Familyboard URL
  │
...
start "" "C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe" --kiosk https://<your-familyboard-url>/ --no-first-run --lang=de
echo [%date% %time%] Edge kiosk started >> %LOGFILE%
```

Replace `https://<your-familyboard-url>/` and `--lang=de` to taste.
...
Replace `<YOUR_DEVICE_ID>` with the device's UUID from Familyboard (Settings → Devices → enable "Presence sensor" toggle on this device → copy the displayed ID).
...
Copy `presence-sensor.py` from the [Familyboard repo](https://github.com/svenger87/familyboard/blob/main/presence-sensor.py) to `C:\presence-sensor.py`.
...
Or set `FAMILYBOARD_URL` in the user environment so the same script works across redeployments. See [[Presence-Sensor]] for full setup details.
...
The Mele auto-logs in as `Calendar`, KioskLauncher fires, Edge opens in kiosk mode pointed at Familyboard. PresenceSensor starts in the background.

## Updating

### Webapp updates

The Mele just runs a browser pointing at your Familyboard host. When you update the host (`./deploy.sh` or `./start.sh restart`), the kiosk picks up the new code on the next page reload (or after a momentary 502 during the webapp container restart).
...
- [[Reference-Build]] — full BOM, frame construction, photos, cabling
- [[Presence-Sensor]] — full LD2410 setup
- [[Kiosk-Linux-Guidance]] — if you want to run on Linux instead
- [[Self-hosting]] — the Familyboard host the kiosk points at
