...
**Two-way sync** of selected Google calendars into the dashboard view. Events created or edited in Familyboard get pushed back to Google; events created in Google flow into Familyboard at the next sync interval. Per-event mapping rules let you assign events to specific family members based on title patterns.
...
- Push events created/edited/deleted in Familyboard back to the chosen Google calendar
...
2. Create a project named `familyboard` (or whatever).
...
5. Authorized redirect URI:
   ...
   - `https://<your-domain>/api/google/callback` for production (your Familyboard URL)
