...
1. **All data is family-scoped.** A `family_id` UUID gates every row via [Row-Level Security](https://supabase.com/docs/guides/database/postgres/row-level-security). See [[Database-Schema]].
2. **Devices join families via 6-character join codes.** Stored in `families.join_code`. Auth model is "device fingerprint + join code" — see [[Security-and-Threat-Model]].
...
- `/api/presence` — POST endpoint for the LD2410 sensor ([[Presence-Sensor]]).
...
- **No user accounts.** Identity is the device + family-code pair. See [[Security-and-Threat-Model]] for the rationale.
...
- [[Database-Schema]] — table-by-table reference
- [[Self-hosting]] — production deployment specifics
- [[Plugin-Authoring]] — how niche integrations will be packaged in v1.1
