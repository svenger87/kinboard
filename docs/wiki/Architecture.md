...
- **next-intl** for i18n. Cookie-based locale switching, no `[locale]` URL prefix. EN + DE shipped; see [Themes](Themes.md) for adding more.
- **Framer Motion** for the slide/fade transitions across pages.

## Data flow

1. **All data is family-scoped.** A `family_id` UUID gates every row via [Row-Level Security](https://supabase.com/docs/guides/database/postgres/row-level-security). See [Database-Schema](Database-Schema.md).
2. **Devices join families via 6-character join codes.** Stored in `families.join_code`. Auth model is "device fingerprint + join code" — see [Security-and-Threat-Model](Security-and-Threat-Model.md).
...
- `/api/google/*` — OAuth flow + sync ([Google-Calendar](Google-Calendar.md)).
- `/api/homeassistant/*` — HA REST proxy with the family's stored token ([Home-Assistant](Home-Assistant.md)).
...
- `/api/presence` — POST endpoint for the LD2410 sensor ([Presence-Sensor](Presence-Sensor.md)).
...
- **No user accounts.** Identity is the device + family-code pair. See [Security-and-Threat-Model](Security-and-Threat-Model.md) for the rationale.
...
- [Database-Schema](Database-Schema.md) — table-by-table reference
- [Self-hosting](Self-hosting.md) — production deployment specifics
- [Plugin-Authoring](Plugin-Authoring.md) — how niche integrations will be packaged in v1.1
