...
- The auth boundary: device-fingerprint + family-join-code (RLS in Postgres is intentionally disabled — the device-cookie + join-code model is the actual auth boundary; treating Familyboard as if RLS were enforced is a misconfiguration, not a vulnerability)
